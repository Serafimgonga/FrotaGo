...
public class VehiclesController : ControllerBase
{
    ...
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateVehicleCommand command)
    {
        if (id != command.Id)
        {
            return BadRequest(new { message = "O ID no URL não coincide com o ID do corpo do pedido." });
        }

        try
        {
            var success = await _mediator.Send(command);
            if (!success) return NotFound(new { message = "Veículo não encontrado." });
    ...
    public async Task<IActionResult> Delete(Guid id)
    {
        try
        {
            ...
            if (!success) return NotFound(new { message = "Veículo não encontrado." });
            return Ok(new { message = "Veículo eliminado com sucesso." });
        }
        catch (System.Exception ex)
        {
            return BadRequest(new { message = ex.Message });
